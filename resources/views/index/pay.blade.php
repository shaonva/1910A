    ...
    <div class="maincont">
     ...
     <div class="dingdanlist" onClick="window.location.href='address.html'">
      <table>
       ...
       @foreach($cart as $v)
       <tr>
        <td class="dingimg" width="15%"><img src="{{env('UPLOADS_URLL')}}{{$v->goods_img}}" /></td>
        <td width="50%">
         <h3>{{$v->goods_name}}</h3>
         <time>下单时间：{{date('Y-d-d H:i:s',$v->add_time)}}</time>
        </td>
    ...
    <div class="gwcpiao">
     <table>
      <tr>
       ...
       <td width="40%"><a href="{{url('/pay_success')}}" class="jiesuan">提交订单</a></td>
      </tr>
     </table>
    </div><!--gwcpiao/-->
    </div><!--maincont-->
